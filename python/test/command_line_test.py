...
class CommandLineTest(unittest.TestCase):
    ...
    def setUp(self):
        self.dataset_filename = config.VmafConfig.test_resource_path('example_dataset.py')
        self.raw_dataset_filename = config.VmafConfig.test_resource_path('example_raw_dataset.py')
        self.out_model_filepath = config.VmafConfig.workdir_path('tmp.pkl')
        self.param_filename = config.VmafConfig.test_resource_path('vmaf_v4.py')
        self.batch_filename = config.VmafConfig.workdir_path('test_batch_input')
    ...
    def test_run_testing_vmaf(self):
        exe = config.VmafConfig.root_path('run_testing')
    ...
    def test_run_vmaf_training(self):
        exe = config.VmafConfig.root_path('run_vmaf_training')
    ...
    def test_run_vmaf_in_batch(self):
        line = 'yuv420p 576 324 {root}/python/test/resource/yuv/src01_hrc00_576x324.yuv ' \
               '{root}/python/test/resource/yuv/src01_hrc01_576x324.yuv'.format(root=config.VmafConfig.root_path())
        cmd = 'echo "{line}" > {batch_filename}'.format(
            line=line, batch_filename=self.batch_filename)
        ret = run_process(cmd, shell=True)
        self.assertEquals(ret, 0)

        exe = config.VmafConfig.root_path('run_vmaf_in_batch')
    ...
    def test_run_vmaf(self):
        exe = config.VmafConfig.root_path('run_vmaf')
        line = 'yuv420p 576 324 {root}/python/test/resource/yuv/src01_hrc00_576x324.yuv ' \
               '{root}/python/test/resource/yuv/src01_hrc01_576x324.yuv'.format(root=config.VmafConfig.root_path())
    ...
    def test_run_psnr(self):
        exe = config.VmafConfig.root_path('run_psnr')
        line = 'yuv420p 576 324 {root}/python/test/resource/yuv/src01_hrc00_576x324.yuv ' \
               '{root}/python/test/resource/yuv/src01_hrc01_576x324.yuv'.format(root=config.VmafConfig.root_path())
        cmd = "{exe} {line} >/dev/null 2>&1".format(line=line, exe=exe)
        ret = run_process(cmd, shell=True)
    ...
    def test_run_cleaning_cache_psnr(self):
        ...
        exe = config.VmafConfig.root_path('python', 'script', 'run_cleaning_cache.py')
