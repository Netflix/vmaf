...
class Executor(TypeVersionEnabled):
    ...
    def _open_workfile(cls, asset, path, workfile_path, yuv_type, workfile_yuv_type, decoder_type: Optional[str], preresampling_filterchain: Optional[List[str]],
        ...
        if fifo_mode:
            if os.path.exists(workfile_path):
                if logger is not None:
                    logger.info(f"Removing existing FIFO or file at {workfile_path}.")
                os.remove(workfile_path)
            if logger is not None:
                logger.info(f"Creating FIFO at {workfile_path}.")
        ...
        if open_sem is not None:
            if logger is not None:
                logger.info(f"Releasing semaphore: signaling ready for {workfile_path}.")
            open_sem.release()
