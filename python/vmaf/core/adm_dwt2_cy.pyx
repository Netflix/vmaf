...
cdef extern from "../../../libvmaf/src/feature/adm_tools.c":
    ...
    void adm_dwt2_s(const float *src, const adm_dwt_band_t *dst, int **ind_y, int **ind_x, int w, int h, int src_stride, int dst_stride)

cdef extern from "../../../libvmaf/src/feature/adm.c":
    char *init_dwt_band(adm_dwt_band_t *band, char *data_top, size_t buf_sz_one)
...
def adm_dwt2_cy(np.ndarray[np.float32_t, ndim=2, mode='c'] a):
    ...
    cdef np.ndarray[np.float32_t, ndim=2, mode='c'] out_a, out_v, out_h, out_d

    try:
        dwt2_src_indices_filt_s(ind_y, ind_x, w, h)
        adm_dwt2_s(aa, &aa_dwt2, ind_y, ind_x, w, h, curr_ref_stride, buf_stride)
        ...
        out_h[...] = np.asarray(<np.float32_t[:h_new, :w_new]> aa_dwt2.band_h)[...]
