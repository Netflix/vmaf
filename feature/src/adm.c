...
int compute_adm(const number_t *ref, const number_t *dis, int w, int h, int ref_stride, int dis_stride, double *score, double *score_num, double *score_den, double *scores)
{
#ifdef ADM_OPT_SINGLE_PRECISION
...
#endif
		num_scale += adm_sum_cube(cm_r.band_h, w, h, buf_stride, 0.1);
		num_scale += adm_sum_cube(cm_r.band_v, w, h, buf_stride, 0.1);
		num_scale += adm_sum_cube(cm_r.band_d, w, h, buf_stride, 0.1);

		den_scale += adm_sum_cube(csf_o.band_h, w, h, buf_stride, 0.1);
		den_scale += adm_sum_cube(csf_o.band_v, w, h, buf_stride, 0.1);
		den_scale += adm_sum_cube(csf_o.band_d, w, h, buf_stride, 0.1);
