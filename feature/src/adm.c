...
static int compute_dlm(const number_t *ref, const number_t *dis, int w, int h, int ref_stride, int dis_stride, double *score)
{
#ifdef ADM_OPT_SINGLE_PRECISION
...
int adm(const char *ref_path, const char *dis_path, int w, int h, const char *fmt)
{
	...
	while ((!feof(ref_rfile)) && (!feof(dis_rfile)))
	{
		...
		if ((ret = compute_dlm(ref_buf, dis_buf, w, h, stride, stride, &score)))
		{
			printf("error: compute_dlm failed.\n");
			fflush(stdout);
			goto fail_or_end;
		}
