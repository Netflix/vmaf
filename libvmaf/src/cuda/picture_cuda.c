...
int vmaf_cuda_picture_download_async(VmafPicture *cuda_pic, VmafPicture *pic, uint8_t bitmask)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if ((bitmask >> i) & 1)
        {
            CHECK_CUDA(cuMemcpy2DAsync(&m, cuda_priv->str));
        }
    }
    return CUDA_SUCCESS;
}
                             ...
                             VmafPicture *pic, uint8_t bitmask)
{
    ...
    CHECK_CUDA(cuEventRecord(cuda_priv->ready, cuda_priv->str));
    return CUDA_SUCCESS;
}
...
static int default_release_pinned_picture(VmafPicture *pic, void *cookie)
{
    ...
    CHECK_CUDA(cuCtxPushCurrent(priv->ctx));
...
int vmaf_cuda_picture_alloc(VmafPicture *pic, void *cookie)
{
    ...
    VmafCudaCookie* cuda_cookie = (VmafCudaCookie*)cookie;
    ...
    priv->ctx = cuda_cookie->state->ctx;
    CHECK_CUDA(cuStreamCreate(&priv->str, CU_STREAM_DEFAULT));
    CHECK_CUDA(cuEventCreate(&priv->ready, CU_EVENT_DEFAULT));
    CHECK_CUDA(cuEventCreate(&priv->finished, CU_EVENT_DEFAULT));
    CHECK_CUDA(cuEventRecord(priv->finished, priv->str));
...
int vmaf_cuda_picture_free(VmafPicture *pic, void *cookie)
{
    ...
    VmafCudaCookie* cuda_cookie = (VmafCudaCookie*) cookie;
    CHECK_CUDA(cuCtxPushCurrent(cuda_cookie->state->ctx));
    CHECK_CUDA(cuStreamSynchronize(priv->str));

    for (int i = 0; i < 3; i++) {
	    if (pic->data[i])
		    CHECK_CUDA(cuMemFreeAsync(pic->data[i], priv->str));
    }
    CHECK_CUDA(cuEventDestroy(priv->finished));
    CHECK_CUDA(cuEventDestroy(priv->ready));
    CHECK_CUDA(cuStreamDestroy(priv->str));
    CHECK_CUDA(cuCtxPopCurrent(NULL));
    free(priv);
    memset(pic, 0, sizeof(*pic));
    return 0;
...
{
    VmafPicturePrivate* priv = pic->priv;
    CHECK_CUDA(cuEventSynchronize(priv->finished));
    CHECK_CUDA(cuStreamSynchronize(priv->str));
