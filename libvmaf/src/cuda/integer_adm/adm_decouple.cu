...
__global__ void adm_decouple_kernel(AdmBufferCuda buf, int top, int bottom,
  ...
  if (i < bottom && j < right) {
    ...
    // the result of 2*int16_t needs 31 bits. adding two 31 bit values results in 32 bits.
...
which lie outside the frame borders */
  ...
  dim3 block(8, 8);
  dim3 grid(DIV_ROUND_UP(right - left, block.x),
            DIV_ROUND_UP(bottom - top, block.y));

  adm_decouple_s123_kernel<<<grid, block, 0, c_stream>>>(
      *buf, top, bottom, left, right, stride,
...
void adm_decouple_device(AdmBufferCuda *buf, int w, int h, int stride, AdmFixedParametersCuda* p,
  ...
  adm_decouple_kernel<<<grid, block, 0, c_stream>>>(
      *buf, top, bottom, left, right, stride,
      p->adm_enhn_gain_limit);
  CudaCheckError();
}
