...
__global__ void calculate_motion_score_kernel_16bpc(const VmafPicture src, CudaVmafBuffer src_blurred, 
  ...
  constexpr unsigned shift_var = 16u;
  constexpr unsigned add_before_shift = 32768u; 
...
#pragma unroll
      ...
      blurred += filter_d[xf]*((blurred_y + add_before_shift) >> shift_var);
    }

    blurred = (blurred + add_before_shift) >> shift_var;
    reinterpret_cast<uint16_t*>(src_blurred.data + y*blurred_stride)[x] = static_cast<uint16_t>(blurred);
    abs_dist = abs(static_cast<int>(blurred)-static_cast<int>(reinterpret_cast<uint16_t*>(prev_blurred.data + y*blurred_stride)[x]));
  }
