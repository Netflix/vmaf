 ...
 *
 *  Integer VIF feature extractor — Vulkan backend (T5-1b-iv-vif).
 *
 *  Canonical name: integer_vif_vulkan (matches the CUDA convention:
 *  feature/cuda/integer_vif_cuda.c → feature/vulkan/integer_vif_vulkan.c).
 *  The previous filename vif_vulkan.c is retained as a build alias for
 *  source-compatibility during the transition; integer_vif_vulkan.c is
 *  the authoritative TU going forward.
 *
