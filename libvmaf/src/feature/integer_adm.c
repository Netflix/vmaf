                       ...
                       bool measure_aim)
{
    ...
    // DEFERRED: adm_p_norm is hard-coded to 3.0f because changing it requires
    // retraining the VMAF model.  If model retraining is planned, the lines
    // below show the parameterised form (adm_p_norm replaces the literal 3.0f):
    // This would mean:
    ...
    for (int i = 0; i < (h + 1) / 2; ++i) {
        ...
        for (int j = 0; j < (w + 1) / 2; ++j) {
            ...
            i4_ref_dwt2->band_a[i * dst_stride + j] =
                (int32_t)((accum_ref + add_bef_shift_round_HP[scale - 1]) >>
                          shift_HorizontalPass[scale - 1]);
    ...
    for (unsigned scale = 0; scale < 4; ++scale) {
        ...
        if (scale == 0) {
            if (adm_skip_scale0) {
                ...
                if (ref_pic->bpc == 8) {
                    adm_dwt2_8_lo(ref_pic->data[0], &buf->ref_dwt2, buf, w, h, curr_ref_stride,
                                  buf_stride);
                    adm_dwt2_8_lo(dis_pic->data[0], &buf->dis_dwt2, buf, w, h, curr_dis_stride,
                                  buf_stride);
                } else {
                    adm_dwt2_16_lo(ref_pic->data[0], &buf->ref_dwt2, buf, w, h, curr_ref_stride,
                                   buf_stride, ref_pic->bpc);
                    adm_dwt2_16_lo(dis_pic->data[0], &buf->dis_dwt2, buf, w, h, curr_dis_stride,
                                   buf_stride, dis_pic->bpc);
                }
                ...
                if (ref_pic->bpc == 8) {
                    s->dwt2_8(ref_pic->data[0], &buf->ref_dwt2, buf, w, h, curr_ref_stride,
                              buf_stride);
                    s->dwt2_8(dis_pic->data[0], &buf->dis_dwt2, buf, w, h, curr_dis_stride,
                              buf_stride);
                } else {
                    s->dwt2_16(ref_pic->data[0], &buf->ref_dwt2, buf, w, h, curr_ref_stride,
                               buf_stride, ref_pic->bpc);
                    s->dwt2_16(dis_pic->data[0], &buf->dis_dwt2, buf, w, h, curr_dis_stride,
                               buf_stride, dis_pic->bpc);
                }
                ...
                den_scale = s->adm_csf_den_scale(
                    &buf->ref_dwt2, w, h, buf_stride, adm_norm_view_dist, adm_ref_display_height,
                    adm_csf_mode, adm_csf_scale, adm_csf_diag_scale, adm_noise_weight);

                s->adm_csf(buf, w, h, buf_stride, adm_norm_view_dist, adm_ref_display_height,
                           adm_csf_mode, adm_csf_scale, adm_csf_diag_scale, false);
                ...
                if (!adm_skip_aim) {
                    s->adm_csf(buf, w, h, buf_stride, adm_norm_view_dist, adm_ref_display_height,
                               adm_csf_mode, adm_csf_scale, adm_csf_diag_scale, true);
            ...
            s->adm_dwt2_s123_combined(i4_curr_ref_scale, i4_curr_dis_scale, buf, w, h,
                                      curr_ref_stride, curr_dis_stride, buf_stride, scale);
            ...
            den_scale =
                s->adm_csf_den_s123(&buf->i4_ref_dwt2, scale, w, h, buf_stride, adm_norm_view_dist,
                                    adm_ref_display_height, adm_csf_mode, adm_csf_scale,
                                    adm_csf_diag_scale, adm_noise_weight);

            s->i4_adm_csf(buf, scale, w, h, buf_stride, adm_norm_view_dist, adm_ref_display_height,
                          adm_csf_mode, adm_csf_scale, adm_csf_diag_scale, false);
                   ...
                   VmafFeatureCollector *feature_collector)
{
    ...
    err |= vmaf_feature_collector_append_with_dict(
        ...
        MAX(score * s->adm_dlm_weight + (1 - score_aim) * (1 - s->adm_dlm_weight), s->adm_min_val),
        index);
