                   ...
                   unsigned index, VmafFeatureCollector *feature_collector)
{
    ...
    err |= vmaf_feature_collector_append_with_options(feature_collector,
            score, index, "VMAF_integer_feature_adm2_score", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[0] / scores[1], index, "integer_adm_scale0", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[2] / scores[3], index, "integer_adm_scale1", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[4] / scores[5], index, "integer_adm_scale2", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[6] / scores[7], index, "integer_adm_scale3", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    if (!s->debug) return err;

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            score, index, "integer_adm", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            score_num, index, "integer_adm_num", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            score_den, index, "integer_adm_den", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[0], index, "integer_adm_num_scale0", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[1], index, "integer_adm_den_scale0", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[2], index, "integer_adm_num_scale1", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[3], index, "integer_adm_den_scale1", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[4], index, "integer_adm_num_scale2", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[5], index, "integer_adm_den_scale2", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[6], index, "integer_adm_num_scale3", fex->options, s,
            1, &s->adm_enhn_gain_limit);

    err |= vmaf_feature_collector_append_with_options(feature_collector,
            scores[7], index, "integer_adm_den_scale3", fex->options, s,
            1, &s->adm_enhn_gain_limit);
