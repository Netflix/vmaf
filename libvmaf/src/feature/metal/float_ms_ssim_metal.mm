...
typedef struct FloatMsSsimStateMetal {
    ...
    /* Pyramid buffers: 5 levels x ref + cmp (float, Shared storage). */
     ...
     * Sized to block_count[i] = ceil(w_f[i]/16) x ceil(h_f[i]/8). */
                          ...
                          unsigned bpc, unsigned w, unsigned h)
{
...

    {
        ...
        /* hbuf: 5 planes x w_h[0] x H[0] floats. */
        {
                            ...
                            VmafPicture *dist_pic_90, unsigned index)
{
    ...
    /* Build pyramid scales 1..4 via decimate kernel (x ref + cmp). */
