 ...
 *
 *  1. `ms_ssim_decimate` — 9-tap biorthogonal 9/7 separable LPF +
 *     2x downsample with period-2n mirror boundary (one call per
 *     inter-scale transition x 2 sides = 8 dispatches for 5 levels).
 *     Input/output: float buffers (scale 0 is normalised host pixels).
 *     Grid: ceil(w_out/16) x ceil(h_out/16).
 *
 *  2. `ms_ssim_horiz` — horizontal 11-tap separable Gaussian over the
 *     five SSIM statistics on a float input pyramid level. Output
 *     width = W - 10, same height. Grid: ceil(w_h/16) x ceil(H/8).
 *
 *  3. `ms_ssim_vert_lcs` — vertical 11-tap on the horiz-pass
 *     intermediates + per-pixel L/C/S formulas + per-threadgroup
 *     float partial sums x 3. Grid: ceil(w_f/16) x ceil(h_f/8).
 ...
 *   - sigma^2 clamped to >= 0 before sqrt (matches fmaxf in the CUDA
 *     twin and MAX(0, ...) in iqa/ssim_tools.c::ssim_variance_scalar).
 *   - Covariance sign-fix for the S component: when sigma_xy_geom <= 0
 ...
 *
 *  Min-dim guard (ADR-0153): 11 x 2^4 = 176 — enforced in init() of
 *  the .mm host.
 */
...
/*                                                                      */
/*  Buffer bindings:                                                    */
/*   [[buffer(0)]] src     — const float * input plane (w_in x h_in)   */
/*   [[buffer(1)]] dst     — float * output plane (w_out x h_out)      */
/*   [[buffer(2)]] dims    — uint4 (.x=w_in, .y=h_in,                  */
/*                                   .z=w_out, .w=h_out)               */
/*  Grid: ceil(w_out/16) x ceil(h_out/16),  threads 16x16.            */
/* ------------------------------------------------------------------ */
...
/*                                                                      */
/*  Buffer bindings:                                                    */
/*   [[buffer(0)]] ref_in  — const float * reference (width x height)  */
/*   [[buffer(1)]] cmp_in  — const float * distorted (width x height)  */
/*   [[buffer(2)]] hbuf    — float * 5-plane intermediate (w_h x H x5) */
/*   [[buffer(3)]] params  — uint4 (.x=width, .y=height,               */
/*                                   .z=w_h, .w=0)                     */
/*  Grid: ceil(w_h/16) x ceil(height/8),  threads 16x8.               */
/* ------------------------------------------------------------------ */
    ...
    uint2  gid [[thread_position_in_grid]])
{
    ...
    /* Five planes, each w_h x height floats. */
...
/*                                                                      */
/*  Buffer bindings:                                                    */
/*   [[buffer(0)]] hbuf        — const float * (5 x w_h x H from K2)  */
...
/*  Grid: ceil(w_f/16) x ceil(h_f/8),  threads 16x8.                  */
/* ------------------------------------------------------------------ */
    ...
    uint   simd_count [[simdgroups_per_threadgroup]])
{
    ...
    if (x < w_f && y < h_f) {
        ...
        for (int v = 0; v < 11; ++v) {
            ...
            const float wv = G[v];
            mu_r += wv * hbuf[0u * plane_stride + src_idx];
            mu_c += wv * hbuf[1u * plane_stride + src_idx];
            sq_r += wv * hbuf[2u * plane_stride + src_idx];
            sq_c += wv * hbuf[3u * plane_stride + src_idx];
            rc   += wv * hbuf[4u * plane_stride + src_idx];
        }

        /* sigma^2 clamped to >= 0 — matches CUDA twin fmaxf and iqa ref. */
        ...
        /* Covariance sign-fix: when sigma_geom <= 0 and covariance < 0,
