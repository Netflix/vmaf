...
/**
 * Works similar to floating-point function convolution_f32_c_s
 */
...
int integer_compute_motion(const int16_t *ref, const int16_t *dis, int w, int h, int ref_stride, int dis_stride, double *score)
{
    ...
    if (ref_stride % sizeof(int16_t) != 0)
    {
        printf("error: ref_stride %% sizeof(int16_t) != 0, ref_stride = %d, sizeof(int16_t) = %zu.\n", ref_stride, sizeof(int16_t));
        fflush(stdout);
        goto fail;
    }
    if (dis_stride % sizeof(int16_t) != 0)
    {
        printf("error: dis_stride %% sizeof(int16_t) != 0, dis_stride = %d, sizeof(int16_t) = %zu.\n", dis_stride, sizeof(int16_t));
        fflush(stdout);
        goto fail;
    }
    // stride for integer_vmaf_image_sad_c is in terms of (sizeof(int16_t) bytes)
    *score = integer_vmaf_image_sad_c(ref, dis, w, h, ref_stride / sizeof(int16_t), dis_stride / sizeof(int16_t));
